...
**Fuente de verdad backend:** `docs/PROJECT_MAP.md`, Swagger `/api/docs`, PRDs F-02 a F-13.
...
Primera pantalla: pegar solo el **Prompt 0 — Design System Global**.
Cada pantalla siguiente: pegar **Prompt 0 (abreviado)** + el prompt de la pantalla. En Stitch, referencia visual: "Mantener el mismo design system que la pantalla Home".
Orden recomendado: 0 → A1 → A2 → A3 → A4 → 1 → 2 → 3 → 18 → 19 → 20 → 21 → resto por tab.
Formato: cada prompt está en inglés (mejor rendimiento en herramientas de diseño IA); las notas entre corchetes [ES: ...] son para ti, no las pegues en Stitch.

> [!IMPORTANT]
> **Componentes Atómicos y Reutilización de Layouts:**
> Todos los componentes de la interfaz deben ser atómicos. **No se debe repetir la barra de navegación inferior (footer/bottom tab bar) ni la barra superior (navbar/app bar) en cada vista.** Estos elementos de navegación globales deben ser diseñados e implementados una sola vez a nivel de Layout base o shell de la aplicación, inyectando de forma dinámica el contenido de cada pantalla específica en su sección central correspondiente.
...
- Registrar token FCM tras login en `src/notifications/notifications.controller.ts`.
